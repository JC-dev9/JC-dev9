# 👋 Olá! Eu sou Juan Loza

💻 Apaixonado por tecnologia |
📍 Portugal | 🌱 Sempre aprendendo algo novo

---

## 🚀 Sobre mim

Sou um jovem desenvolvedor em constante evolução, focado em criar soluções inteligentes e úteis com código limpo e boas práticas. Tenho experiência com:

- 🧠 **Inteligência Artificial**: desenvolvimento de chatbots, automações com Python, extração de dados via OCR, aplicação básica de machine learning e NLP (Natural Language Processing)
- 🌐 **Desenvolvimento Web**: HTML, CSS, JavaScript e React.js
- 🗃️ **Base de dados**: MySQL, SQL Server, PostgreSQL e SQLite
- 🧰 **Back-end e APIs**: Express.js, Node.js, PHP, C, C# e integração com APIs RESTful

---

## 🛠️ Tecnologias e Ferramentas

![My Skills](https://skillicons.dev/icons?i=html,css,js,c,cs,sql,react,nextjs,nodejs,express,dotnet,flutter,python,php,mysql,sqlite,postgres,sqlserver,supabase,git,github,vscode,visualstudio,vercel,azure,linux,windows)



---
## 📫 Como falar comigo

- Email: [juanloza.dev@gmail.com](mailto:juanloza.dev@gmail.com)
- LinkedIn: [linkedin.com/in/juan-loza](https://pt.linkedin.com/in/juan-loza-5b549b285)
- Portfólio (em breve)

---

💡 *"Tudo posso naquele que me fortalece." – Filipenses 4:13*

